...
if(isset($_POST['sub']))
{
    $c=$_POST['crs'];
    $d=$_POST['dep'];
    header("location:staff.php?d=$d&c=$c");
}
if(isset($_GET['delid']))
{
    $c=$_GET['c'];
    ...
    if($del>0)
    {
        header("location:staff.php?c=$c&d=$d&sid=$sid");
    } 
}
if(isset($_GET['delstid']))
{
    ...
    $d=$_GET['d'];
    $sid=$_GET['sid'];
    $del1=mysqli_query($dbcon,"delete from user_log where uid='$sid'");
        if($del1>0)
        {
            $up=mysqli_query($dbcon,"update staf_data set st=0 where stfif='$sid'");
            if($up>0)
            {
                header("location:staff.php?c=$c&d=$d");
            }               
        }
}
if(isset($_POST['sub1']))
{
    ...
    {
        $dd=explode(":", $v);
        $ins_as=mysqli_query($dbcon,"INSERT INTO `subjct_assign`(`assignid`, `stf_nme`, `stf_id`, `crs`, `dep`, `sem`, `subnme`, `subid`, `st`) VALUES ('','$sn','$sid','$crs','$dep','$sem','$dd[0]','$dd[1]','1')");
    }
    header("location:staff.php?sid=".$_GET['sid']."&d=$dep&c=$crs");
}
...
<html lang="en">
                                                        ...
                                                        <div class="col-lg-6">
                                                            ...
                                                            if(isset($_GET['c']))
                                                            {
                                                               ...
                                                               $rd1=mysqli_fetch_row($d1);
                                                                $sel_stf=mysqli_query($dbcon,"select * from staf_data where crs='$c' and dep='$d' and st=1");
                                                                ...
                                                                <table class="table table-bordered table-condensed table-responsive">
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <td><a href="staff.php?d=<?php echo $d ?>&c=<?php echo $c ?>&sid=<?php echo $r_stf[2] ?>"><span class="glyphicon glyphicon-search" style="color:green; cursor: pointer;"></span></a></td>
                                                                    </tr>
                                                                    <?php
                                                                    }
                                                                    ?>
    ...
    $r_st=mysqli_fetch_row($sel_st);
                                                                ?>
                                                            <a href="staff.php"><span class="label label-danger pull_right">BACK</span></a>
                <table class="table table-bordered table-condensed table-hover table-striped">
                <tr>
                    <td style="width: 50%"><center>
                        <img src="../staffpic/<?php echo $r_st[10] ?>" class="img img-responsive img-thumbnail" />
                        
                        <a href="staff.php?sid=<?php echo $_GET['sid'] ?>&delstid=1&d=<?php echo $_GET['d'] ?>&c=<?php echo $_GET['c']?>" class="label label-danger">Delete Staff</a>
          ...
          <script>
            ...
            {             
                var c=document.getElementById("crs1").value;
                ...
                xmlhttp.open("GET", "loadsubject.php?c="+c+"&d="+d+"&s="+s+"&sid=<?php echo $_GET['sid'] ?>", true);
                xmlhttp.send();
            }
            </script>
                ...
                <td>
                    ...
                    Qualification <?php echo $r_st[9] ?><hr />
