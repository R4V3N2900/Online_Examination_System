...
{
    $nt=$_POST['nt'];
    $desr=  addslashes(nl2br($_POST['desr']));
    $sr=$_POST['sr'];
    $dt=$_POST['dt'];
    $ins_job=mysqli_query($dbcon,"INSERT INTO `job_vacancy`(`id`, `not_tit`, `descr`, `skl_req`, `lst_dt`, `post_dt`, `st`) VALUES ('','$nt','$desr','$sr','$dt','".date('Y-m-d')."','1')");
    if($ins_job>0)
    {
...
if(isset($_GET['jid']))
{
    $up=mysqli_query($dbcon,"update job_vacancy set st=0 where id='".$_GET['jid']."'");
    if($up>0)
    {
         header("location:index.php");
    }
					...
					<div class="page-content">
						...
						<div class="row">
                                                    <div class="col-xs-12">
                                                        ...
                                                        <div class="col-md-6">
                                                            <?php
                                                            $sel_job=mysqli_query($dbcon,"select * from job_vacancy where st=0");
                                                            if(mysqli_num_rows($sel_job)>0)
                                                            {
                                                                ?>
                                                            <table class="table table-bordered table-condensed table-responsive">
                                                                ...
                                                                <?php
                                                                $i=0;
                                                                while($rjob=mysqli_fetch_row($sel_job))
                                                                {
                                                                    $i++;
                                                                    ?>
                                                                <tr>
                                                                    <td><?php echo $i ?></td>
                                                                    <td><?php echo $rjob[1] ?></td>
                                                                    <td>
                                                                        <a href="jobs.php?view=<?php echo $rjob[0] ?>"><span class="label label-danger"> 
                                                                        <?php
                                                                        $sel_req=mysqli_query($dbcon,"select * from vacancy_req where vac_id='$rjob[0]'");
                                                                        echo mysqli_num_rows($sel_req);
                                                                        ?> Response Found</span></a>
                                                                    </td>
                                                        ...
                                                        <div class="col-md-6">
                                                            ...
                                                            if(isset($_GET['view']))
                                                            {
                                                               $selj1=mysqli_query($dbcon,"select * from job_vacancy where id='".$_GET['view']."'");
                                                               $rj=mysqli_fetch_row($selj1);
                                                               ?>
                                                            <table class="table table-bordered table-condensed table-responsive">
                                                                <tr>
                                                                    <td><a href="index.php" class="label label-danger pull-right">BACK</a><h3><?php echo $rj[1] ?></h3></td>
                                                                </tr>
                                                                <tr>
                                                                    <td><b>JOB DESCRIPTION</b><p style="text-align: justify; padding: 5px; text-indent: 50px;"><?php echo $rj[2] ?></p>
                                                                        <b>SKILL REQUIRED</b>
                                                                        <ul>
                                                                        <?php
                                                                        $sk=  explode(",", $rj[3]);
                                                                ...
                                                                <tr>
                                                                    <td><h5><b>Interested Students</b></h5>
                                                                    <?php
                                                                    $sel_is=mysqli_query($dbcon,"select * from vacancy_req,student_data where student_data.admnum=vacancy_req.adm_num and vacancy_req.vac_id='".$_GET['view']."'");
                                                                    if(mysqli_num_rows($sel_is)>0)
                                                                    {
                                                                        echo"<ul>";
                                                                        while($ris=mysqli_fetch_row($sel_is))
                                                                        {
                                                                            echo"</li>$ris[12]</li>";
                                                                        }
                                                                        echo"</ul>";
