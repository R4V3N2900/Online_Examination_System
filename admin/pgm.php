			...
			<div class="navbar-container" id="navbar-container">
				...
				<div class="navbar-buttons navbar-header pull-right" role="navigation">
					<ul class="nav ace-nav">
						...
						<li class="purple">
                                                    <?php
                                                    $sel1=mysqli_query($dbcon,"select * from pgm_data where pgm_dt>'".date('Y-m-d')."'");
                                                    ?>
							<a data-toggle="dropdown" class="dropdown-toggle" href="#">
								<i class="ace-icon fa fa-bell icon-animated-bell"></i>
								<span class="badge badge-important"><?php echo mysqli_num_rows($sel1); ?></span>
							</a>

							<ul class="dropdown-menu-right dropdown-navbar navbar-pink dropdown-menu dropdown-caret dropdown-close">
								<li class="dropdown-header">
									<i class="ace-icon fa fa-exclamation-triangle"></i>
									<?php echo mysqli_num_rows($sel1); ?> New Program
								</li>

								<li class="dropdown-content">
									<ul class="dropdown-menu dropdown-navbar navbar-pink">
                                                                            <?php
                                                                            $sel2=mysqli_query($dbcon,"select * from pgm_data order by id desc limit 5");
                                                                            if(mysqli_num_rows($sel2)>0)
                                                                            {
                                                                                while($r2=mysqli_fetch_row($sel2))
                                                                                {
                                                                            ?>
										<li>
											<?php echo $r2[1] ?>
				...
				<div class="main-content-inner">
					...
					<div class="page-content">
						...
						<div class="row">
                                                    <div class="col-xs-12">
                                                        <?php
                                                            $sel_pg=mysqli_query($dbcon,"select * from pgm_data order by id desc");
                                                            if(mysqli_num_rows($sel_pg)>0)
                                                            {
                                                                ?>
                                                            <table class="table table-bordered table-condensed table-responsive">
                                                                ...
                                                                <?php
                                                                $i=0;
                                                                while($rpg=mysqli_fetch_row($sel_pg))
                                                                {
                                                                    $i++;
                                                                    ?>
                                                                <tr>
                                                                    <td><?php echo $i ?></td>
                                                                    <td><?php echo $rpg[1] ?></td>
                                                                    <td><?php echo $rpg[2] ?></td>
                                                                    <td><?php echo date("d-M-Y",strtotime($rpg[3])) ?></td>
